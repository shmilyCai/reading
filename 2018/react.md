...
Virtual DOM，是JavaScript对象，即一棵树

当数据更新时，diff寻找到需要变更的DOM节点，并只对变化的部分进行实际的浏览器DOM更新，而不是重新渲染整个DOM树。	

Virtual DOM机制是将更新的数据实时反应到UI上，即就树与新树的比较，更新树后，再通过Patch更新到DOM中
...
React中的组件，其实就是有限状态机，通过状态渲染对应的界面，且每个组件生命周期，它规定了组件的状态和方法需要在哪个阶段改变和执行。
